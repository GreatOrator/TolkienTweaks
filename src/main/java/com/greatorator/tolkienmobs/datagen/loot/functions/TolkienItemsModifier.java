...
public class TolkienItemsModifier extends LootModifier {

    public static final MapCodec<TolkienItemsModifier> CODEC = RecordCodecBuilder.mapCodec(inst -> codecStart(inst)
            .and(Codec.unboundedMap(Codec.STRING, ExtraCodecs.POSITIVE_FLOAT)
                    .fieldOf("items")
                    .forGetter(addItemModifier -> addItemModifier.addItems)
            ).apply(inst, TolkienItemsModifier::new));


    Map<String, Float> addItems;


    public TolkienItemsModifier(LootItemCondition[] conditionsIn, Map<String, Float> addItems) {
        super(conditionsIn);
