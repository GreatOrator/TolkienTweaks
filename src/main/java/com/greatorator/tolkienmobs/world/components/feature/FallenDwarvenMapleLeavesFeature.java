...
public class FallenDwarvenMapleLeavesFeature extends Feature<NoneFeatureConfiguration> {

	public FallenDwarvenMapleLeavesFeature(Codec<NoneFeatureConfiguration> config) {
		super(config);
	}

	private final BlockState state = TolkienBlocks.LEAFPILE_DWARVEN_MAPLE.get().defaultBlockState();
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> ctx) {
		...
		if (this.canPlace(position, level)) {
			if (!level.getFluidState(position.below()).isEmpty()) {
				...
				level.setBlock(position, this.state.setValue(LeafPileBlock.LAYERS, startHeight), 16 | 2);
	...
	private void checkAndGenerateLeafPile(WorldGenLevel level, BlockPos pos, int pileLayer) {
		...
		if (this.state.canSurvive(level, finalPos))
			level.setBlock(finalPos, this.state.setValue(LeafPileBlock.LAYERS, pileLayer), 16 | 2);
	}

	private boolean canPlace(BlockPos pos, WorldGenLevel level) {
		BlockState state = level.getBlockState(pos.below());
		return !level.getBlockState(pos).is(this.state.getBlock()) && (level.isEmptyBlock(pos) || level.getBlockState(pos).canBeReplaced()) && (state.is(BlockTags.DIRT) || level.getFluidState(pos.below()).is(Fluids.WATER));
	}
}
