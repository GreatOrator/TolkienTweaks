...
@OnlyIn(Dist.CLIENT)
public class CamoSpawnerRenderer implements BlockEntityRenderer<CamoSpawnerBlockEntity> {
    private final EntityRenderDispatcher entityRenderer;

    public CamoSpawnerRenderer(BlockEntityRendererProvider.Context context) {
        this.entityRenderer = context.getEntityRenderer();
    }

    public void render(CamoSpawnerBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        Level level = blockEntity.getLevel();
        if (level != null) {
            TolkienBaseSpawner basespawner = blockEntity.getSpawnerBase();
    ...
    @Override
    public net.minecraft.world.phys.AABB getRenderBoundingBox(CamoSpawnerBlockEntity blockEntity) {
