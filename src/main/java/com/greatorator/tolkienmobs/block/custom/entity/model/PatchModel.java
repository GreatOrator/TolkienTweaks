...
import com.greatorator.tolkienmobs.block.TolkienPatchBlock;
...
public record PatchModel(TextureAtlasSprite texture, boolean shaggify) implements BakedModel {
	...
	public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, RandomSource random) {
		if (state == null)
			...
			return this.getQuads(state.getValue(TolkienPatchBlock.NORTH), state.getValue(TolkienPatchBlock.EAST), state.getValue(TolkienPatchBlock.SOUTH), state.getValue(TolkienPatchBlock.WEST), random);
	}

	private List<BakedQuad> getQuads(boolean north, boolean east, boolean south, boolean west, RandomSource posRandom) {
		List<BakedQuad> list = new ArrayList<>();

		BoundingBox bb = TolkienPatchBlock.AABBFromRandom(posRandom);
	...
	public ChunkRenderTypeSet getRenderTypes(@NotNull BlockState state, @NotNull RandomSource rand, @NotNull ModelData data) {
		if (state.is(TolkienBlocks.CLOVER_PATCH)) {
